...
Recorder::~Recorder() {
    for (auto lib_iter = recorder_map.begin(); lib_iter != recorder_map.end(); ++lib_iter) {
        std::string lib_name = lib_iter->first;
        std::cout << ">>>>>>>>>> Library: " << lib_name << std::endl;
        for (auto func_iter = lib_iter->second.begin(); func_iter != lib_iter->second.end(); ++func_iter) {
            std::cout << func_iter->first << ": " << func_iter->second << std::endl;
        }
    }
}
...
void Tracer::trace() {
    ...
    if (dladdr(addr, &info)) {
        ...
        if (Recorder::enable) {
            ...
            if (recorder) { recorder->record(lib_name, func_name); }
        }
    }
