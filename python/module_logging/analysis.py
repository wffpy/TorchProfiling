...
class Analyzer:
    ...
    def gen_detail_table(self):
        ...
        for elem in final_list:
            table.add_row([fill(elem.get_module_name(), width=50), elem.get_name(), elem.get_time()])
...
def merge_block(lhs:Block, rhs:Block):
    ...
    table = pt.PrettyTable([fill("Module Name", width=200), fill("GPU Op", width=100), fill("GPU Time(ms)", width=30), fill("XPU Op", width=100), fill("XPU Time(ms)", width=30)])
    for i in range(max_len):
        if i < len(lhs_op_list) and i < len(rhs_op_list):
            lhs_op = lhs_op_list[i]
            rhs_op = rhs_op_list[i]
            table.add_row([fill(lhs.get_name(), width=200), lhs_op.get_name(), lhs_op.get_time(), rhs_op.get_name(), rhs_op.get_time()])
    ...
    table.add_row([fill("", width=200), "GPU Total", lhs.get_time(), "XPU Total", rhs.get_time()])
    return table
